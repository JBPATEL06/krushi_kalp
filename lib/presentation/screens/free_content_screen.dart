...
class _FreeContentScreenState extends State<FreeContentScreen> {
  ...
  void _onSearchChanged() {
    setState(() {
      ...
      debugPrint('[FreeContent] 🔍 Search query changed: "$_searchQuery"');
    });
  }
  ...
  List<dynamic> _getFilteredItems() {
    ...
    if (_selectedFilter == 'All' || _selectedFilter == 'Tests') {
      items.addAll(testProvider.tests.where((test) => test.price == 0));
    }

    if (_selectedFilter == 'All' || _selectedFilter == 'Resources') {
      items.addAll(resourceProvider.ebooks.where((r) => r.price == 0));
      items.addAll(resourceProvider.studyMaterials.where((r) => r.price == 0));
      items.addAll(resourceProvider.pyqs.where((r) => r.price == 0));
      items.addAll(resourceProvider.currentAffairs.where((r) => r.price == 0));
    }
    ...
    if (_searchQuery.isNotEmpty) {
      items = items.where((item) {
        final title = (item is MockTest ? item.title : (item as Resource).title)
            .toLowerCase();
        return title.contains(_searchQuery);
      }).toList();

      debugPrint('[FreeContent] 🔎 After search: ${items.length} items match');
    }
  ...
  Widget _buildTestCard(MockTest test, int index) {
    ...
    return UniversalItemCard(
      ...
      onActionTap: () {
        debugPrint('[FreeContent] ▶️ Starting test: ${test.id}');
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => MockTestDetailScreen(
  ...
  Widget _buildResourceCard(Resource resource, int index) {
    ...
    return UniversalItemCard(
      ...
      onActionTap: () {
        debugPrint('[FreeContent] 📥 Opening resource: ${resource.id}');
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => ResourceDetailScreen(
