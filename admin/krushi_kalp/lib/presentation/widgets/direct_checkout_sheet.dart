...
class _DirectCheckoutSheetState extends State<DirectCheckoutSheet> {
  ...
  Future<void> _initiatePurchase() async {
    ...
    try {
      ...
      // Fetch Phone Number for Razorpay Prefill
      ...
      if (userPhone == null || userPhone.isEmpty || userPhone.length < 10) {
        try {
          final profile = await Supabase.instance.client
              ...
              .eq('id', user.id)
              .maybeSingle();
          final dbPhone = profile?['phonenumber'] as String?;
          if (dbPhone != null && dbPhone.length >= 10) {
            userPhone = dbPhone;
          }
        } catch (_) {}
      }

      // Cleanup: Razorpay often prefers Just 10 digits for Indian numbers
      if (userPhone != null) {
      ...
      // Final fallback to ensure the page is skipped
      final contact = (userPhone != null && userPhone.length >= 10)
          ? userPhone
          : '8888888888';

      if (!mounted) return; // Check again before opening checkout

      // 2. Open Razorpay
      _paymentService.openCheckout(
        amount: _finalPrice, // Pay Final Price
        orderId: orderId, // Pass DB Order ID for reference
        email: user.email,
        contact: contact,
      );
