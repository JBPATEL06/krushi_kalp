...
class _ExamScreenState extends State<ExamScreen> {
  ...
  void _submitTest({bool autoSubmit = false}) {
    ...
    if (mounted) {
      // Submit to Backend (Fire and Forget or Await?)
      // Awaiting is safer to ensure data consistency
      final user = AuthService().currentUser;
      if (user != null) {
        TestService.submitTestResult(
          testId: widget.test.id,
          score: totalScore,
          totalMarks: widget.test.totalMarks,
          authUserId: user.id,
          language: widget.examLanguage, // Pass the language used
        ).catchError((e) {
          debugPrint("Background submission failed: $e");
          // Optional: Show snackbar, but don't block navigation
        });
      }
